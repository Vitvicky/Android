...
public class FSNamesystem implements FSConstants, FSNamesystemMBean,
	...
	private static final void logAuditEvent(UserGroupInformation ugi,
		...
		auditLog.info(fmt.format(
				...
				src,
				dst,
				(stat == null) ? null : stat.getOwner() + ':' + stat.getGroup()
	...
	private static final long DELEGATION_TOKEN_REMOVER_SCAN_INTERVAL = TimeUnit.MILLISECONDS
	...
	/**
	 * Store set of Blocks that need to be replicated 1 or more times. Set of:
	 * Block
	 */
	...
	private InetSocketAddress nameNodeAddress = null; // TODO: name-node has
	...
	private void initialize(NameNode nn, Configuration conf) throws IOException {
		...
		NameNode.getNameNodeMetrics()
				.setFsImageLoadTime(timeTakenToLoadFSImage);
	...
	public static Collection<File> getNamespaceEditsDirs(Configuration conf) {
		Collection<String> editsDirNames = conf
				.getStringCollection("dfs.name.edits.dir");
	...
	private void setConfigurationParameters(Configuration conf)
		...
		short filePermission = (short) conf.getInt("dfs.upgrade.permission",
				0777);
		this.defaultPermission = PermissionStatus.createImmutable(fsOwner
				.getShortUserName(), supergroup, new FsPermission(
				filePermission));

		this.replicator = new ReplicationTargetChooser(conf.getBoolean(
				"dfs.replication.considerLoad", true), this, clusterMap);
		this.blocksInvalidateWorkPct = DFSUtil
		...
		this.replicationRecheckInterval = conf.getInt(
				"dfs.replication.interval", 3) * 1000L;
		this.defaultBlockSize = conf.getLong("dfs.block.size",
				DEFAULT_BLOCK_SIZE);
		...
		if (isAccessTokenEnabled) {
			this.accessKeyUpdateInterval = conf
					.getLong(
							DFSConfigKeys.DFS_BLOCK_ACCESS_KEY_UPDATE_INTERVAL_KEY,
							600) * 60 * 1000L; // 10 hrs
			this.accessTokenLifetime = conf.getLong(
					DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_LIFETIME_KEY, 600) * 60 * 1000L; // 10
																							// hrs
		}
		...
		if (this.checkForStaleNodes) {
			...
			if (this.staleInterval < DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_MILLI_DEFAULT) {
				LOG.warn("The given interval for marking stale datanode = "
						+ this.staleInterval
						+ ", which is smaller than the default value "
						+ DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_MILLI_DEFAULT
						+ ".");
			}
		}
		...
		synchronized (neededReplications) {
			...
			for (Block block : neededReplications) {
				...
				if (block instanceof BlockInfo) {
					String fileName = FSDirectory
							.getFullPathName(((BlockInfo) block).getINode());
					out.print(fileName + ": ");
				}

				// l: == live:, d: == decommissioned c: == corrupt e: == excess
				out.print(block + ((usableReplicas > 0) ? "" : " MISSING")
						+ " (replicas:" + " l: " + numReplicas.liveReplicas()
						+ " d: " + numReplicas.decommissionedReplicas()
						+ " c: " + numReplicas.corruptReplicas() + " e: "
						+ numReplicas.excessReplicas() + ") ");

				Collection<DatanodeDescriptor> corruptNodes = corruptReplicas
						.getNodes(block);

				for (Iterator<DatanodeDescriptor> jt = blocksMap
						.nodeIterator(block); jt.hasNext();) {
	...
	synchronized void updateNeededReplications(Block block,
		...
		neededReplications.update(block, repl.liveReplicas(),
				repl.decommissionedReplicas(), curExpectedReplicas,
	...
	private long addBlock(Block block, List<BlockWithLocations> results) {
		ArrayList<String> machineSet = new ArrayList<String>(
	...
	public void setOwner(String src, String username, String group)
		...
		synchronized (this) {
			if (isInSafeMode())
				throw new SafeModeException("Cannot set owner for " + src,
						safeMode);
	...
	public LocatedBlocks getBlockLocations(String src, long offset,
		...
		final LocatedBlocks ret = getBlockLocationsInternal(src, offset,
				length, Integer.MAX_VALUE, doAccessTime, needBlockToken);
		if (auditLog.isInfoEnabled() && isExternalInvocation()) {
			logAuditEvent(UserGroupInformation.getCurrentUser(),
		...
		if (checkSafeMode && isInSafeMode()) {
			for (LocatedBlock b : ret.getLocatedBlocks()) {
				// if safemode & no block locations yet then throw
				// safemodeException
				if ((b.getLocations() == null)
						|| (b.getLocations().length == 0)) {
					throw new SafeModeException("Zero blocklocations for "
							+ src, safeMode);
				}
			}
		}
	 ...
	 * under-replicated data blocks or removal of the eccessive block copies if
	...
	private synchronized boolean setReplicationInternal(String src,
		...
		if (oldRepl > replication) {
			...
			for (int idx = 0; idx < fileBlocks.length; idx++)
				processOverReplicatedBlock(fileBlocks[idx], replication, null,
						null);
	 ...
	 */
	private void verifyReplication(String src, short replication,
	...
	void startFile(String src, PermissionStatus permissions, String holder,
		...
		startFileInternal(src, permissions, holder, clientMachine, overwrite,
				false, createParent, replication, blockSize);
		getEditLog().logSync();
		if (auditLog.isInfoEnabled() && isExternalInvocation()) {
			final HdfsFileStatus stat = dir.getFileInfo(src);
			logAuditEvent(UserGroupInformation.getCurrentUser(),
	...
	private synchronized void startFileInternal(String src,
			...
			boolean overwrite, boolean append, boolean createParent,
			short replication, long blockSize) throws IOException {
	 ...
	 * @throws IOException
	 */
	synchronized boolean recoverLease(String src, String holder,
	...
	private void recoverLeaseInternal(INode fileInode, String src,
		...
		if (fileInode != null && fileInode.isUnderConstruction()) {
			...
			if (force) {
				...
				LOG.info("recoverLease: recover lease " + lease + ", src="
						+ src + " from client " + pendingFile.clientName);
	...
	LocatedBlock appendFile(String src, String holder, String clientMachine)
		...
		startFileInternal(src, null, holder, clientMachine, false, true, false,
		...
		synchronized (this) {
			// Need to re-check existence here, since the file may have been
			// deleted
			...
			if (blocks != null && blocks.length > 0) {
				...
				if (file.getPreferredBlockSize() > storedBlock.getNumBytes()) {
					...
					DatanodeDescriptor[] targets = new DatanodeDescriptor[blocksMap
							.numNodes(last)];
					Iterator<DatanodeDescriptor> it = blocksMap
							.nodeIterator(last);
					for (int i = 0; it != null && it.hasNext(); i++) {
						targets[i] = it.next();
					}
					// remove the replica locations of this block from the
					// blocksMap
					...
					lb = new LocatedBlock(last, targets, fileLength
							- storedBlock.getNumBytes());
					if (isAccessTokenEnabled) {
						lb.setBlockToken(accessTokenHandler.generateToken(lb
								.getBlock(), EnumSet
					...
					for (DatanodeDescriptor dd : targets) {
						...
						Collection<Block> v = recentInvalidateSets
								.get(datanodeId);
	...
	void addToInvalidatesNoLog(Block b, DatanodeInfo n) {
		Collection<Block> invalidateSet = recentInvalidateSets.get(n
				.getStorageID());
	...
	private synchronized void dumpRecentInvalidateSets(PrintWriter out) {
		...
		for (Map.Entry<String, Collection<Block>> entry : recentInvalidateSets
				.entrySet()) {
	...
	public synchronized void markBlockAsCorrupt(Block blk, DatanodeInfo dn)
		...
		if (storedBlockInfo == null) {
			...
			// thread of Datanode reports bad block before Block reports are
			// sent
			// by the Datanode on startup
			NameNode.stateChangeLog
					.info("BLOCK NameSystem.markBlockAsCorrupt: " + "block "
			...
			if (inode == null) {
				NameNode.stateChangeLog
						.info("BLOCK NameSystem.markBlockAsCorrupt: "
			...
			if (countNodes(storedBlockInfo).liveReplicas() > inode
					.getReplication()) {
	...
	private synchronized void invalidateBlock(Block blk, DatanodeInfo dn)
		...
		if (count > 1) {
			...
			NameNode.stateChangeLog
					.debug("BLOCK* NameSystem.invalidateBlocks: " + blk
							+ " on " + dn.getName() + " listed for deletion.");
		} else {
	...
	/**
	 * Remove the indicated filename from the namespace. This may invalidate
	 * some blocks that make up the file.
	 */
	synchronized boolean deleteInternal(String src, boolean enforcePermission)
			throws IOException {
		if (NameNode.stateChangeLog.isDebugEnabled()) {
	...
	/**
	 * This is invoked when a lease expires. On lease expiry, all the files that
	 * were written from that dfsclient should be recovered.
	 */
	...
	void internalReleaseLeaseOne(Lease lease, String src) throws IOException {
		...
		Lease reassignedLease = reassignLease(lease, src,
				HdfsConstants.NN_RECOVERY_LEASEHOLDER, pendingFile);
		...
		synchronized (this) {
			if (isInSafeMode()) {
				throw new SafeModeException(
						"Cannot commitBlockSynchronization " + lastblock,
						safeMode);
			}
			...
			if (deleteblock) {
				...
				// update last block, construct newblockinfo and add it to the
				// blocks map
				lastblock.set(lastblock.getBlockId(), newlength,
						newgenerationstamp);
				final BlockInfo newblockinfo = blocksMap.addINode(lastblock,
						pendingFile);

				// find the DatanodeDescriptor objects
				// There should be no locations in the blocksMap till now
				// because the
				...
				for (int i = 0; i < newtargets.length; i++) {
					DatanodeDescriptor node = datanodeMap.get(newtargets[i]
							.getStorageID());
					if (node != null) {
						if (closeFile) {
							// If we aren't closing the file, we shouldn't add
							// it to the
							// block list for the node, since the block is still
							// under
							// construction there. (in getAdditionalBlock, for
							// example
			...
			if (!closeFile) {
				...
				LOG.info("commitBlockSynchronization(" + lastblock
						+ ") successful");
				return;
			}
	...
	class ReplicationMonitor implements Runnable {
			...
			while (fsRunning) {
				try {
					...
					LOG.warn("ReplicationMonitor thread received exception. "
							+ ie + " " + StringUtils.stringifyException(ie));
				} catch (Throwable t) {
					LOG.warn("ReplicationMonitor thread received Runtime exception. "
							+ t + " " + StringUtils.stringifyException(t));
		 ...
		 * {@link ReplicationMonitor} starts processing these queues via calls
		 * to {@link #computeDatanodeWork()}. Each call does a chunk of work,
		 * then waits 3 seconds before doing the next chunk of work, to avoid
		 * monopolizing the CPUs and the global lock. It may take several cycles
		 * before the queue is completely flushed.
		 * 
		 * Here we use two concrete subclasses of
		 * {@link QueueProcessingStatistics} to collect stats about these
		 * processes.
		 */
		private class ReplicateQueueProcessingStats extends
			...
			// @see
			// org.apache.hadoop.hdfs.server.namenode.FSNamesystem.QueueProcessingStatistics#preCheckIsLastCycle(int)
			@Override
			...
			// @see
			// org.apache.hadoop.hdfs.server.namenode.FSNamesystem.QueueProcessingStatistics#postCheckIsLastCycle(int)
			@Override
		...
		private class InvalidateQueueProcessingStats extends
				QueueProcessingStatistics {
	...
	int computeInvalidateWork(int nodesToProcess) {
		...
		if (nodesToProcess < remainingNodes) {
			...
			for (int i = 0; i < remainingNodes; i++) {
				...
				Collections.swap(keyArray, keyIndex, numOfNodes - i - 1); // swap
																			// to
																			// end
			}
		}
	...
	boolean computeReplicationWorkForBlock(Block block, int priority) {
		...
		synchronized (this) {
			synchronized (neededReplications) {
				...
				srcNode = chooseSourceDatanode(block, containingNodes,
						numReplicas);
				if ((numReplicas.liveReplicas() + numReplicas
		...
		DatanodeDescriptor targets[] = replicator.chooseTarget(
				requiredReplication - numEffectiveReplicas, srcNode,
				containingNodes, null, block.getNumBytes());
		if (targets.length == 0)
			return false;
			...
			synchronized (neededReplications) {
				...
				if (NameNode.stateChangeLog.isInfoEnabled()) {
					...
					NameNode.stateChangeLog.info("BLOCK* ask "
							+ srcNode.getName() + " to replicate " + block
							+ " to " + targetList);
					NameNode.stateChangeLog
							.debug("BLOCK* neededReplications = "
									+ neededReplications.size()
									+ " pendingReplications = "
	...
	public DatanodeInfo chooseDatanode(String address, long blocksize) {
		final DatanodeDescriptor clientNode = host2DataNodeMap
				.getDatanodeByHost(address);
		if (clientNode != null) {
			final DatanodeDescriptor[] datanodes = replicator.chooseTarget(1,
	...
	private DatanodeDescriptor chooseSourceDatanode(Block block,
			List<DatanodeDescriptor> containingNodes, NumberReplicas numReplicas,int choose) {
		...
		while (it.hasNext()) {
			...
			if ((nodesCorrupt != null) && nodesCorrupt.contains(node))
				//nodelist.add(node.getStorageID());
				continue;
			...
			if (node.isDecommissionInProgress() || srcNode == null||nodelist==null) {
				
				srcNode = node;
				nodelist.add(node);
				continue;
			}
			if (srcNode.isDecommissionInProgress()||node.isDecommissionInProgress())
				nodelist.remove(node.getStorageID())
				continue;
			...
			synchronized (this) {
				for (int i = 0; i < timedOutItems.length; i++) {
					...
					neededReplications.add(timedOutItems[i],
							num.liveReplicas(), num.decommissionedReplicas(),
	...
	public synchronized void processBlocksBeingWrittenReport(DatanodeID nodeID,
		...
		for (int i = 0; i < blocksBeingWritten.getNumberOfBlocks(); i++) {
			...
			BlockInfo storedBlock = blocksMap
					.getStoredBlockWithoutMatchingGS(block);
	...
	public synchronized void processReport(DatanodeID nodeID,
		...
		if (node == null || !node.isAlive) {
			throw new IOException(
					"ProcessReport from dead or unregisterted node: "
							+ nodeID.getName());
		}
		...
		for (Block b : toInvalidate) {
			NameNode.stateChangeLog
					.info("BLOCK* NameSystem.processReport: block " + b
							+ " on " + node.getName() + " size "
	...
	synchronized Block addStoredBlock(Block block, DatanodeDescriptor node,
		...
		if (storedBlock == null) {
			// If we have a block in the block map with the same ID, but a
			// different
			// generation stamp, and the corresponding file is under
			// construction,
			...
			boolean reportedOldGS = block.getGenerationStamp() < storedBlock
					.getGenerationStamp();
			boolean reportedNewGS = block.getGenerationStamp() > storedBlock
					.getGenerationStamp();
			boolean underConstruction = inode.isUnderConstruction();
			boolean isLastBlock = inode.getLastBlock() != null
					&& inode.getLastBlock().getBlockId() == block.getBlockId();
			...
			// Don't add blocks to the DN when they're part of the in-progress
			// last block
			// and have an inconsistent generation stamp. Instead just add them
			// to targets
			...
			if (underConstruction && isLastBlock
				...
				NameNode.stateChangeLog
						.info("BLOCK* NameSystem.addStoredBlock: "
								+ "Targets updated: block " + block + " on "
								+ node.getName()
								+ " is added as a target for block "
								+ storedBlock + " with size "
								+ block.getNumBytes());
		...
		// Is the block being reported the last block of an underconstruction
		// file?
		...
		if (fileINode.isUnderConstruction()) {
			...
			if (last == null) {
				...
				// than to throw
				// an NPE below.
				LOG.error("Null blocks for reported block=" + block
						+ " stored=" + storedBlock + " inode=" + fileINode);
				return block;
			}
			blockUnderConstruction = last.equals(storedBlock);
		}

		// block == storedBlock when this addStoredBlock is the result of a
		// block report
		if (block != storedBlock) {
			if (block.getNumBytes() >= 0) {
				...
				if (cursize == 0) {
					...
					try {
						if (cursize > block.getNumBytes()
							...
							// new replica is smaller in size than existing
							// block.
							// Mark the new replica as corrupt.
							LOG.warn("Mark new replica "
									+ block
									+ " from "
									+ node.getName()
							...
							// new replica is larger in size than existing
							// block.
				...
				long diff = (file == null) ? 0
						: (file.getPreferredBlockSize() - storedBlock
								.getNumBytes());
				...
				if (diff > 0 && file.isUnderConstruction()
					...
					try {
						...
						leaseManager
								.findPath((INodeFileUnderConstruction) file);
						dir.updateSpaceConsumed(path, 0,
								-diff * file.getReplication());
					} catch (IOException e) {
	...
	void invalidateCorruptReplicas(Block blk) {
		...
		for (Iterator<DatanodeDescriptor> it = nodes.iterator(); it.hasNext();) {
			...
			try {
				...
				NameNode.stateChangeLog
						.info("NameNode.invalidateCorruptReplicas "
								+ "error in deleting bad block " + blk + " on "
								+ node + e);
				gotException = true;
			}
		}
	...
	void chooseExcessReplicates(Collection<DatanodeDescriptor> nonExcess,
		...
		while (nonExcess.size() - replication > 0) {
			...
			if (firstOne
				...
				Iterator<DatanodeDescriptor> iter = priSet.isEmpty() ? remains
						.iterator() : priSet.iterator();
				while (iter.hasNext()) {
					DatanodeDescriptor node = iter.next();
			...
			if (excessBlocks.add(b)) {
				excessBlocksCount++;
				NameNode.stateChangeLog
						.debug("BLOCK* NameSystem.chooseExcessReplicates: "
								+ "(" + cur.getName() + ", " + b
	...
	synchronized void removeStoredBlock(Block block, DatanodeDescriptor node) {
		...
		if (!blocksMap.removeNode(block, node)) {
			NameNode.stateChangeLog
					.debug("BLOCK* NameSystem.removeStoredBlock: " + block
	...
	public synchronized void blockReceived(DatanodeID nodeID, Block block,
		...
		if (node == null || !node.isAlive) {
			NameNode.stateChangeLog.warn("BLOCK* NameSystem.blockReceived: "
					+ block + " is received from dead or unregistered node "
					+ nodeID.getName());
			throw new IOException(
		...
		if (delHint != null && delHint.length() != 0) {
			...
			if (delHintNode == null) {
				NameNode.stateChangeLog
						.warn("BLOCK* NameSystem.blockReceived: "
								+ block
   ...
   */
	...
	private void logBlockReplicationInfo(Block block,
		...
		FSNamesystem.LOG.info("Block: " + block + ", Expected Replicas: "
				...
				+ ", Datanodes having this block: " + nodeList
				+ ", Current Datanode: " + srcNode.name
		...
		for (final Iterator<Block> i = srcNode.getBlockIterator(); i.hasNext();) {
			...
			if (fileINode != null) {
				...
				if (curExpectedReplicas > curReplicas) {
					// Log info about one block for this node which needs
					// replication
					if (!status) {
						status = true;
					...
					if (!neededReplications.contains(block)
						...
						// after the startDecommission method has been executed.
						// These
						// blocks were in flight when the decommission was
						// started.
						//
						neededReplications.add(block, curReplicas,
	 ...
	 */
	private synchronized boolean verifyNodeRegistration(
	 ...
	 * {@link #extension} is passed. Then it leaves safe mode and destroys
	 * itself.
	 * <p>
	...
	class SafeModeInfo {
		...
		/**
		 * Ratio of the number of safe blocks to the total number of blocks to
		...
		String getTurnOffTip() {
			...
			return safeBlockRatioMsg + " in "
					+ Math.abs(reached + extension - now()) / 1000
					+ " seconds.";
		}
		...
		public String toString() {
			String resText = "Current safe block ratio = "
					+ getSafeBlockRatio() + ". Target threshold = " + threshold
		...
		boolean isConsistent() throws IOException {
			...
			int activeBlocks = blocksMap.size()
					- (int) pendingDeletionBlocksCount;
	 ...
	 * "hadoop:service=NameNode,name=FSNamesystemState" Register the
	 * FSNamesystem MXBean using the name
	...
	synchronized long nextGenerationStampForBlock(Block block, boolean fromNN)
		...
		BlockInfo storedBlock = blocksMap.getStoredBlock(block);
		if (storedBlock == null) {
			BlockInfo match = blocksMap.getStoredBlock(block
					.getWithWildcardGS());
			String msg = (match == null) ? block + " is missing" : block
					+ " has out of date GS " + block.getGenerationStamp()
		...
		if (!fileINode.isUnderConstruction()) {
			String msg = block
					+ " is already commited, !fileINode.isUnderConstruction().";
			LOG.info(msg);
			throw new IOException(msg);
		}
		...
		if (!fromNN
				&& HdfsConstants.NN_RECOVERY_LEASEHOLDER.equals(leaseManager
						.getLeaseByPath(FSDirectory.getFullPathName(fileINode))
						.getHolder())) {
			String msg = block
		...
		if (!((INodeFileUnderConstruction) fileINode)
				.setLastRecoveryTime(now())) {
			String msg = block
					+ " is already being recovered, ignoring this request.";
			LOG.info(msg);
			throw new IOException(msg);
		}
	...
	private DelegationTokenSecretManager createDelegationTokenSecretManager(
		...
		return new DelegationTokenSecretManager(conf.getLong(
				...
				24 * 60 * 60 * 1000), conf.getLong(
				"dfs.namenode.delegation.token.max-lifetime", 7 * 24 * 60 * 60
						* 1000), conf.getLong(
				"dfs.namenode.delegation.token.renew-interval",
				24 * 60 * 60 * 1000), DELEGATION_TOKEN_REMOVER_SCAN_INTERVAL,
				this);
	}
	...
	public String getLiveNodes() {
		...
		final ArrayList<DatanodeDescriptor> aliveNodeList = this
	...
	public String getDeadNodes() {
		final Map<String, Object> info = new HashMap<String, Object>();
		final ArrayList<DatanodeDescriptor> deadNodeList = this
				.getDatanodeListForReport(DatanodeReportType.DEAD);
	...
	public String getDecomNodes() {
		final Map<String, Object> info = new HashMap<String, Object>();
		final ArrayList<DatanodeDescriptor> decomNodeList = this
	...
	public String getNameDirStatuses() {
		...
		for (Iterator<StorageDirectory> it = getFSImage().dirIterator(); it
				.hasNext();) {
		...
		List<Storage.StorageDirectory> removedStorageDirs = getFSImage()
	...
	public void getMetrics(MetricsBuilder builder, boolean all) {
		builder.addRecord("FSNamesystem")
				.setContext("dfs")
	 ...
	 * invocation is internal to the namenode. Client invoked methods are
	 * invoked over RPC and always have address != null.
	 */
	...
	void logFsckEvent(String src, InetAddress remoteAddress) throws IOException {
		if (auditLog.isInfoEnabled()) {
			logAuditEvent(UserGroupInformation.getCurrentUser(), remoteAddress,
					"fsck", src, null, null);
		}
	}
	 ...
	 * 
	 * @param dead
	 *            , array list of dead nodes
	 */
	void removeDecomNodeFromDeadList(ArrayList<DatanodeDescriptor> dead) {
		// If the include list is empty, any nodes are welcomed and it does not
		// make sense to exclude any nodes from the cluster. Therefore, no
		// remove.
		...
		for (Iterator<DatanodeDescriptor> it = dead.iterator(); it.hasNext();) {
			...
			if ((!inHostsList(node, null))
					...
					&& node.isDecommissioned()) {
				// Include list is not empty, an existing datanode does not
				// appear
				// in both include or exclude lists and it has been
				// decommissioned.
